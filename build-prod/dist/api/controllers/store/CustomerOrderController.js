...
let CustomerOrderController = class CustomerOrderController {
  ...
  customerCheckout(checkoutParam, response, request) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
      ...
      const countryName = yield this.countryService.findOneById(
      ...
      for (i = 0; i < orderProduct.length; i++) {
        ...
        productDetails.total =
          +orderProduct[i].quantity * +orderProduct[i].price;
        productDetails.model = orderProduct[i].model;
        const productInformation = yield this.orderProductService.create(
          productDetails
        );
        const productData = yield this.productService.findOneById(
          productInformation.productId
        );
        const productImageDetail = yield this.productImageService.findOne({
        ...
        totalProductAmount = yield this.orderProductService.list({
          where: {
        ...
        productDetailData.push(
          Object.assign(Object.assign({}, productData), {
      ...
      newOrder.orderPrefixId = setting.invoicePrefix.concat(
        '-' + orderDate + orderData.orderId
      );
      const resultData = yield this.orderService.update(
        orderData.orderId,
        newOrder
      );
      ...
      const emailContent = yield this.emailTemplateService.findOneById(5);
      const adminEmailContent = yield this.emailTemplateService.findOneById(6);
      const today =
        ('0' + nowDate.getDate()).slice(-2) +
        '.' +
        ('0' + (nowDate.getMonth() + 1)).slice(-2) +
      ...
      const adminMessage = adminEmailContent.content
        .replace('{name}', customerName)
        .replace('{orderId}', orderData.orderId);
      const customerMessage = emailContent.content.replace(
        '{name}',
        customerName
      );
      mail_services_1.MAILService.adminOrderMail(
        adminMessage,
        orderData,
      ...
      const sendMailRes = mail_services_1.MAILService.customerOrderMail(
        customerMessage,
        orderData,
      ...
      if (sendMailRes) {
        const successResponse = {
          status: 1,
          message:
            'You successfully checked out the product and order details send to your mail',
          data: resultData,
        };
  ...
  orderList(limit, offset, count, request, response) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
      ...
      const OrderData = yield this.orderService.list(options);
      const promises = OrderData.map(results =>
        tslib_1.__awaiter(this, void 0, void 0, function* () {
          const Id = results.orderId;
          const countValue = yield this.orderProductService.findAndCount({
            where: { orderId: Id },
          });
...
tslib_1.__decorate(
  [
    ...
    tslib_1.__metadata('design:paramtypes', [
      customerCheckoutRequest_1.CustomerCheckoutRequest,
      Object,
      Object,
    ]),
    tslib_1.__metadata('design:returntype', Promise),
  ],
  CustomerOrderController.prototype,
  'customerCheckout',
  null
);
tslib_1.__decorate(
